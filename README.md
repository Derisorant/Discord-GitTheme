<div align=center><h1>GitTheme</h1>
<p>
A simple Discord theme recolour inspired by <a href=https://github.com/MarkChan0225/RoundedDiscord>RoundedDiscord</a> and <a href=https://github.com/mwittrien/BetterDiscordAddons/tree/master/Themes/DiscordRecolor>DiscordRecolour</a>.</p>
<div align="center">
<img alt="Static Badge" src="https://img.shields.io/badge/Release%20-%20v1.1%20-%204bc61d?style=flat&label=Release&link=https%3A%2F%2Fgithub.com%2FDerisorant%2FDiscord-GitTheme%2Freleases%2Ftag%2Fv1.1"> <!-- Update Better Discord download url when it's uploaded! -->
<a href=https://betterdiscord.app/><img src="https://img.shields.io/badge/Better-Discord-3a71c1?logo=data:image/svg+xml;base64,&style=flat-square&labelColor=black"></img></a>
<img alt="GitHub Downloads (all assets, all releases)" src="https://img.shields.io/github/downloads/Derisorant/Discord-GitTheme/total?label=Downloads&link=https%3A%2F%2Fgithub.com%2FDerisorant%2FDiscord-GitTheme%2Freleases%2Flatest">

</div> 
</div>


## Download 📩

<!--* From [BetterDiscord](NotCurrentlyUploaded)  Update Better Discord download url when it's uploaded! -->
* From [Releases](https://github.com/Derisorant/Discord-GitTheme/releases/latest)

## Usage 🤚
### [BetterDiscord](https://betterdiscord.app/)
After downloading, place the `GitTheme.css` file in your `Themes` folder. If you don't know how to locate the themes folder, follow this [Guide](https://betterdiscord.app/FAQ#:~:text=Open%20your%20discord%20user%20settings,"Open%20themes%20folder"%20button.).

### [VenCord](https://vencord.dev/)
After downloading, place the `GitTheme.css` file in your `Themes` folder. This can be found in `Discord -> Settings -> Themes -> Open Themes Folder`

## Preview 📷
<img src="https://github.com/Derisorant/Discord-GitTheme/blob/main/Assets/Fullscreen.png?raw=true">
